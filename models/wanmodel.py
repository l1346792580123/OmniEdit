...
from .wanutils import flash_attention
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        if e.dim() == 3:
            ...
            e = [item.squeeze(2) for item in e]
        ...
        def cross_attn_ffn(x, context, context_lens, e):
            ...
            if self.use_audio:
                x = self.audio_cross_attn_wrapper(x, audio, seq_lens, grid_sizes, freqs, audio_seq_len, pad_shape)
...
class WanT2VCrossAttentionGather(WanSelfAttentionSepKVDim):
    ...
    def forward(self, x, context, context_lens, grid_sizes, freqs, audio_seq_len, pad_shape=0):
        r"""
        Args:
...
class AudioCrossAttentionWrapper(nn.Module):
    ...
    def forward(self, x, audio, seq_lens, grid_sizes, freqs, audio_seq_len, pad_shape=0):
        x = x + self.audio_cross_attn(
            self.norm1_audio(x), audio, seq_lens, grid_sizes, freqs, audio_seq_len, pad_shape)
        return x