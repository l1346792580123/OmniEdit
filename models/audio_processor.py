...
class AudioProcessor:
    ...
    def get_audio_feat(self, audio_input, target_fps, frame_num):
        audio_features = []
        window = 750*640
        sampling_rate = 16000
        audio_len = len(audio_input) // 640 # 16000*time/640 对应fps25的视频帧数
        for i in range(0, len(audio_input), window):
            audio_feature = self.feature_extractor(audio_input[i:i+window], sampling_rate=sampling_rate, return_tensors="pt", ).input_features
            audio_features.append(audio_feature)
        audio_features = torch.cat(audio_features, dim=-1)

        audio_feature = audio_features.to(self.whisper.device).float()
        window = 3000
        audio_prompts = []
        for i in range(0, audio_feature.shape[-1], window):
            audio_prompt = self.whisper.encoder(audio_feature[:,:,i:i+window], output_hidden_states=True).hidden_states
        ...
        audio_emb = audio_prompts

        feat0 = linear_interpolation_fps(audio_emb[:, :, 0: 8].mean(dim=2), 50, target_fps)
        feat1 = linear_interpolation_fps(audio_emb[:, :, 8: 16].mean(dim=2), 50, target_fps)
        feat2 = linear_interpolation_fps(audio_emb[:, :, 16: 24].mean(dim=2), 50, target_fps)
        feat3 = linear_interpolation_fps(audio_emb[:, :, 24: 32].mean(dim=2), 50, target_fps)
        feat4 = linear_interpolation_fps(audio_emb[:, :, 32], 50, target_fps)
        feat_merge = torch.stack([feat0, feat1, feat2, feat3, feat4], dim=2)[0]  # [T, 5, 1280]

        audio_emb, _ = self.get_audio_emb_window(feat_merge, frame_num, frame0_idx=0)

        return audio_emb.unsqueeze(0)